...
当前状态：**阶段 1、2 已完成**。阶段 3 起尚未开始。每阶段完成后的交付与验收记录见 [tasks/](tasks/README.md)。

## 需求偏差（已锁定并记录）

- V1 需求中的 `input_price` / `output_price` 与 Dashboard 成本**整体移出 V1**：注册表没有价格列，models.dev 的 `cost` 同步时直接丢弃，Policy 不参与成本计算。原因是用户选择“完全不存价格”，成本口径需要单独设计，不能与路由正确性混在一起。
- 需求 10 的 `api_key` 保留，但**明文存储在 SQLite**（无静态加密），作为已接受的残余风险记录；缓解措施见 [ARCHITECTURE.md](ARCHITECTURE.md)。
...
| 2. Model Registry | models.dev 元数据同步（精确白名单）、本地 Provider/模型覆盖（base_url、api_key、启停、优先级、上游模型标识）、SQLite 持久化、只读快照 | 同模型多 Provider；重复/悬空映射拒绝；容量与标识校验；重启后保留；新增模型只改配置或重跑同步；密钥不进日志；同步失败整体回滚 |
...
| 6. Policy Engine | 硬性能力过滤、可配置 confidence 分层、Provider/模型优先级、用户规则、解释与 fallback | 纯函数表驱动测试；高/中/低 confidence 边界；Provider 禁用/不可用；黑白名单；tools/vision/context 限制；无合格候选；确定性平局；fallback 不能绕过硬性约束 |
...
| 8. Logging | SQLite 路由日志、usage、请求关联、可选 Jev 调试轨迹、脱敏 | 成功/失败/取消/流式均有日志；字段完整；缺失 usage 不虚构；日志失败不污染已发送响应；默认无 prompt/密钥；明确数据保留策略 |
...
| 10. Dashboard | 简单后台：Dashboard、Models、Providers、Logs、Settings | 请求/模型/Provider 分布、Auto Router 命中率、平均 latency、tokens 与数据库一致；可查原因；空态/错误态；表单校验；键盘可用；不暴露密钥 |
...
| 9. 完整路由日志 | 8、9 |
| 10. 解释模型选择原因 | 6、8、10 |
| 11. 新增模型不改 Router 核心 | 2、7、9（阶段 2 已验证：改配置或加白名单后重跑同步即可） |
...
4. **可选外部集成**：显式提供 Bifrost/TypeSafe 配置后运行；测试结果区分真实调用与模拟调用，默认 CI 不花费模型调用费用。阶段 2 只从真实 models.dev 抓取样例并存为固定 fixture，测试本身不访问网络、不调用模型。
